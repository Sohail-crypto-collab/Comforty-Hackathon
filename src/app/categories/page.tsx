...
const Categories = async () => {
  ...
  return (
    ...
    <div className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-8">
      <h2  className="text-[#272343] lg:text-2xl lg:mt-10 lg:mb-16 sm:my-10 text-4xl font-semibold">
        Top Categories
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6  overflow-x-auto scrollbar-hide gap-8 mt-10 pl-5 pr-5 snap-x">
        {categoriesProduct.map((cat: any, index: number) => (
          <Link href={`/categories/${cat.title}`} key={index}>
            <div className="relative  flex-shrink-0 cursor-pointer snap-start">
              <img
                src={cat.imageUrl}
                alt={cat.title}
